...
## 6. 反思
這次實作讓我對「分層排錯」有更清楚的理解。以前遇到連線失敗時，通常只會覺得服務壞掉，但這次發現同樣是 timeout，其實可能來自不同層的問題，例如網路介面關閉或防火牆阻擋。

在 F1 和 F2 的測試中，表面上都是 ssh 無法連線，但透過 ping 測試可以很明顯區分問題發生的位置。如果 ping 不通，代表是網路層的問題；如果 ping 可以但 ssh 不行，就比較可能是防火牆或服務設定的問題。

另外，在設定 bastion 與 app 的架構時，也更理解為什麼實務上會使用跳板機來限制存取範圍，讓內部服務不直接暴露在外網，提升安全性。
## 7. Bonus 1
